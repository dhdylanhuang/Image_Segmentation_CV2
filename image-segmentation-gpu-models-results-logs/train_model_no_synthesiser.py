...
LOG_PATH = RESULTS_ROOT / "train_model_no_synthesiser.log"
logger = logging.getLogger("train_model_no_synthesiser")
...
log_info(f'RESULTS_ROOT: {RESULTS_ROOT}')
log_info(f'LOG_PATH: {LOG_PATH}')
...
log_info(f"Dataset structure:")
log_info(f"  train/  — {len(train_sats)} sat images, {len(train_masks)} masks")
log_info(f"  valid/  — {len(val_sats)} sat images (no masks, inference only)")
log_info(f"  test/   — {len(test_sats)} sat images (no masks, inference only)")

if len(train_sats) != len(train_masks):
    log_info(f"WARNING: sat/mask count mismatch in train/ ({len(train_sats)} vs {len(train_masks)})")
...
log_info(f"Total labeled samples: {len(all_images)}")
...
print(f"Training config -> num_epochs: {num_epochs} | eval metric: {metric_name}")
...
def train_model(model, train_loader, valid_loader, optimizer, scheduler, scaler, device,
    ...
    for epoch in range(num_epochs):
        ...
        log_info(f"Train Loss: {train_loss:.4f} | Val Loss: {val_loss:.4f} | Val mIoU: {mean_iou:.4f} | Val mAcc: {mean_accuracy:.4f}")
        ...
        if mean_iou > best_miou:
            ...
            if save_path:
                ...
                log_info(f"✓ Saved best model to {save_path}")
        else:
            patience_counter += 1
            log_info(f"No improvement. Patience {patience_counter}/{patience}")
...
def train_model(model, train_loader, valid_loader, optimizer, scheduler, scaler, device,
    ...
    if curve_prefix:
        ...
        log_info(f"✓ Loss curves saved to {plot_path}")
        log_info(f"✓ History CSV saved to {csv_path}")
...
for fold, (train_idx, val_idx) in enumerate(kf.split(all_images)):
    log_info(f"\n{'='*55}")
    log_info(f"  Fold {fold + 1} / {K}  —  train: {len(train_idx)}  val: {len(val_idx)}")
    log_info(f"{'='*55}")
    ...
    if best_metrics is None:
        best_metrics = {
            "mean_iou": 0.0,
            "mean_accuracy": 0.0,
            "per_category_iou": [0.0] * len(id2label),
        }
...
log_info(f"\n{'='*55}")
log_info("K-Fold Cross Validation Summary")
log_info(f"{'='*55}")
log_info(f"\n{summary_df.to_string()}")
log_info(f"\nmIoU:       {results_df['mean_iou'].mean():.4f} ± {results_df['mean_iou'].std():.4f}")
log_info(f"mAcc:       {results_df['mean_accuracy'].mean():.4f} ± {results_df['mean_accuracy'].std():.4f}")

recommended_epochs = max(1, round(results_df["best_epoch"].mean()))
log_info(f"Best epoch: {results_df['best_epoch'].tolist()} → recommended for final run: {recommended_epochs}")
...
log_info(f"Training final model for {recommended_epochs} epochs (mean best epoch across folds)")
...
for epoch in range(recommended_epochs):
    ...
    log_info(f"Epoch {epoch + 1}/{recommended_epochs} | Train Loss: {train_loss:.4f}")
...
plt.close()
log_info(f"✓ Final loss curve saved to {RESULTS_ROOT / 'final_curves.png'}")
...
# -----------------------------
# 10) Final training on all 803 samples (no K-fold — run standalone)
# -----------------------------
import pandas as pd
...
for epoch in range(num_epochs):
    train_loss = train_epoch(final_model, final_train_dl, final_optimizer, final_scheduler, final_scaler, device)
    final_history["train_loss"].append(train_loss)
    log_info(f"Epoch {epoch + 1}/{num_epochs} | Train Loss: {train_loss:.4f}")
